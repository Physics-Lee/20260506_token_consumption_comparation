...
> 本项目中 d 和 g 取绝对值——代码 `abs(mean_d / sd_d)`。大多数中文优化 tokenizer（DeepSeek、Qwen）下 mean_d 为负（文言省），abs 只是去负号；少数 tokenizer（Phi-2、GPT-2）下 mean_d 为正（文言反而费），abs 同样出正数。此时效应量大但单边 p 值接近 1——读者可据此判断"差别大但方向反了"。
